...
import { FunctionAnalyzer } from '../../src/functionAnalyzer';
import { DefaultValueType, VariableScope } from '../../src/models/functionMetadata';
...
suite('Function Analyzer Test Suite', () => {
  ...
  return str
    ...
    assert.strictEqual(params[0].defaultValueType, DefaultValueType.Constant);
    assert.strictEqual(params[1].defaultValueType, DefaultValueType.Constant);
    assert.strictEqual(params[2].defaultValueType, DefaultValueType.Expression);
    assert.strictEqual(params[3].defaultValueType, DefaultValueType.Constant);
  });
  ...
  counter++
    ...
    const staticVariables = metadata[0].staticVariables ?? [];
    assert.strictEqual(staticVariables.length >= 2, true);
    
    const counter = staticVariables.find(v => v.name === 'counter');
    assert.ok(counter);
    assert.strictEqual(counter.scopeValue, VariableScope.Static);
